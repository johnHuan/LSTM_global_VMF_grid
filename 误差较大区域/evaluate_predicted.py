...
def assess(basedir, file):
    dataframe = pd.read_csv(basedir + file + '.csv', index_col='epoch',
                            usecols=['epoch', 'true_ztd', 'period_model'])
    spring = dataframe
    y1, y2 = spring['true_ztd'], spring['period_model']
    Ai = y1 - y2
    bias = np.average(Ai)
...
if __name__ == '__main__':
    result_data_dir = "E:/shell/VMF3_OP/zhd_zwd_period_model/误差大区域分析/data/"
    base_dir = "E:/shell/VMF3_OP/zhd_zwd_period_model/误差大区域分析/"
    ...
    for i in files:
        ...
        lat = point[0]
        lng = point[1]
        std, bias, mae, rmse, mse = assess(result_data_dir, point_str)
        data_set.append([lat, lng, std, bias, mae, rmse, mse])
        # j += 1
        # print(j)
    data = pd.DataFrame(data_set)
    data.to_csv(base_dir + "assess.csv")
