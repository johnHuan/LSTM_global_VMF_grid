    ...
    for file in each_year_files:  # each year VMF file
        ...
        for sample in open(current_file):
            if sample.startswith('!'):
                ...
                zhd = float(str.lstrip(sample[38:44]).rstrip())
                zwd = float(str(sample[46:52]).lstrip().rstrip())
                ztd = '%.4f' % (zhd+zwd)
                ...
                content = "{:6.2f}".format(element_x) + ' ' + str(ztd)
                fobj.write(content)
                fobj.write('\n')
                fobj.close()
...
if __name__ == '__main__':
    ...
    target_base_dir = 'E:/shell/VMF3_OP/zhd_zwd_grid/'
