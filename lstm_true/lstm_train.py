...
def lstm_vmf(source_path, source_filename, output_path, year, month):
    ...
    dataset = pd.read_csv(filename, usecols=['c5', 'true_ztd'])
    flag = 0
    ...
    # 第6步 构建数据集, 进行预测
    dataset_total = dataset['true_ztd'][:]
    # 获取输入数据
...
if __name__ == '__main__':
    data_path = "E:/shell/VMF3_OP/period_model/data/"
    output_path = "E:/shell/VMF3_OP/period_model/true_lstm/"
    ...
    try:
        ...
        # lstm_vmf(data_path, grid, output_path, year, month)
        lstm_vmf(data_path, grid, output_path, 2017, month)
