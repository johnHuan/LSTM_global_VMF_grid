...
def assess(basedir, file):
    dataframe = pd.read_csv(basedir + 'data/' + file + '.csv', index_col='epoch',
                            usecols=['epoch', 'true_ztd', 'period_model'])
    y1, y2 = dataframe['true_ztd'], dataframe['period_model']
    Ai = y1 - y2
    bias = np.average(Ai*100)
...
if __name__ == '__main__':
    ...
    data.to_csv(base_dir + "assess-all.csv")
