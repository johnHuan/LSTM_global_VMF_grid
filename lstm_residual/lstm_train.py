...
def plot_predictions(output_path, grid, predict_ztd, test_set):
    ...
    plt.plot(test_set, color='orange', label='ZTD true value')
    plt.plot(predict_ztd_no_system_bias, color='green', label='ZTD predicted value')
    plt.title('LSTM Neural Network ZTD at ' + grid)
    plt.xlabel('epoch')
    plt.ylabel('ZTD')
    plt.legend()  # 给图加图例
...
def lstm_vmf(source_path, source_filename, output_path):
    ...
    # 模型训练
    # x_train.shape: (17172, 360, 1)
    # y_train.shape: (17172, 1)
    # x_test.shape: (1464, 360)
    model.fit(x_train, y_train, epochs=10, batch_size=48)
    model.save(output_path + '/' + str(grid) + '_lstm_ztd.h5')
...
if __name__ == '__main__':
    data_path = "E:/shell/VMF3_OP/zhd_zwd_period_model/data/"
    output_path = "E:/shell/VMF3_OP/zhd_zwd_period_model/residual_lstm/"
    ...
    try:
        ...
        lstm_vmf(data_path, grid, output_path)
